...
def compute_gradient_penalty(D, real_samples, fake_samples, device):
    ...
    # mod: debug code
    # print(real_samples.size(), alpha.shape, interpolates.shape)
...
def train(cfg):
    ...
    transform_imgs = transforms.Compose([
        transforms.ToTensor(),
        # mod: add 0.5 for fourth channel
    ...
    # mod: un_normalize for elevation channel
    un_normalize = UnNormalize((0.5,0.5,0.5,0.5), (0.5,0.5,0.5,0.5))
...
def main():
    ...
    # mod: require_depth=True
    # transfer(G_AB, f"{cfg.DATASETS.SOURCE_PATH}", cfg.DATASETS.TARGET_SIZE,
    #          cfg.OUTPUT_DIR+"/data", torch.device(cfg.MODELS.DEVICE), require_depth=True)
